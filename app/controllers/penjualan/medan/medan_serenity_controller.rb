class Penjualan::Medan::MedanSerenityController < ApplicationController
  include RolesHelper
  ...
  def weekly
    gon.brand = initialize_brand
    gon.max = 800
    @customer_summary = Penjualan::Sale.customer_summary(2, "SERENITY")
    @most_item =  Penjualan::Sale.most_items_ordered_weekly(2, "SERENITY")
    @branch = "MEDAN"
    @brand_name = initialize_brand
  ...
  def monthly
    gon.brand = initialize_brand
    gon.max = 5000
    @customer_summary = Penjualan::Sale.customer_summary_monthly(2, "SERENITY")
    @most_item =  Penjualan::Sale.most_items_ordered_monthly(2, "SERENITY")
    @branch = "MEDAN"
    @brand_name = initialize_brand
