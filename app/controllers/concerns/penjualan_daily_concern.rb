module PenjualanDailyConcern
  ...
  def daily_summary
    ...
    qty_6 = @daily_sum.select {|c| c["date"] == @six_day_ago}
    gon.qty_6 = qty_6.first.nil? ? 0 : qty_6.first.jumlah
    qty_5 = @daily_sum.select {|c| c["date"] == @five_day_ago}
    gon.qty_5 = qty_5.first.nil? ? 0 : qty_5.first.jumlah
    qty_4 = @daily_sum.select {|c| c["date"] == @four_day_ago}
    gon.qty_4 = qty_4.first.nil? ? 0 : qty_4.first.jumlah
    qty_3 = @daily_sum.select {|c| c["date"] == @three_day_ago}
    gon.qty_3 = qty_3.first.nil? ? 0 : qty_3.first.jumlah
    qty_2 = @daily_sum.select {|c| c["date"] == @two_day_ago}
    gon.qty_2 = qty_2.empty? ? 0 : qty_2.first.jumlah
    qty_1 = @daily_sum.select {|c| c["date"] == @last_day}
