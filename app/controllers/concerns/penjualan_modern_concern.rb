module PenjualanModernConcern
  include ApplicationHelper
  extend ActiveSupport::Concern

  ########## MONTHLY
  
  def modern_recap_conc
    @branch_by_brands = Penjualan::Sale.retail_recap_brand(@date, initialize_brach_id)
  end
  
  def modern_nasional_this_month_branches_conc
    @products_monthnas_summary = Penjualan::Sale.modern_nasional_this_month_products(@date, initialize_brand)
    @branches_monthnas_summary = Penjualan::Sale.modern_nasional_this_month_branches(@date, initialize_brand)
    @branches_monthnasstore_summary = Penjualan::Sale.modern_nasional_this_month_branches_store(@date, initialize_brand)
    @monthnas_summary = Penjualan::Sale.modern_nasional_this_month_total(@date, initialize_brand)
  end
  
  def modern_nasional_this_month_conc
    summaries = Penjualan::Sale::modern_nasional_this_month(@date, initialize_brand)
  ...
  def modern_nasional_this_month_branch_conc
    summaries = Penjualan::Sale::modern_nasional_this_month_branch(@date, initialize_brand)
  ...
  def retail_nasional_monthly_conc
    summaries = Penjualan::Sale::modern_nasional_monthly(initialize_brand)
    # target_sum = Penjualan::Sale::target_retail_nasional_monthly(initialize_brand)
    gon.summaries = summaries.map { |u| [Date::ABBR_MONTHNAMES[u.fiscal_month], (u.harga/10000000)] }.to_a
    # gon.target = target_sum.map { |u| [Date::ABBR_MONTHNAMES[u.month], (u.jumlah)] }.to_a
  end
  ...
  def modern_nasional_weekly_branch_conc
    @ret_nas_weekbranch =  Penjualan::Sale::modern_nasional_weekly_branch(initialize_brand)
  end
  
  def modern_nasional_weekly_conc
    summaries =  Penjualan::Sale::modern_nasional_weekly(initialize_brand)
  ...
  def monthly_channel
    ...
    @modern_monthly = Penjualan::Sale.monthly_channel(initialize_brach_id, initialize_brand).select {|c| c["tipecust"] == "DIRECT"}
  ...
  def weekly_channel
    ...
    @modern = Penjualan::Sale.weekly_channel(initialize_brach_id, initialize_brand).select {|c| c["tipecust"] == "DIRECT"}
