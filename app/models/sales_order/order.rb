class SalesOrder::Order < ActiveRecord::Base
  ...
  def self.outstand_order_all(branch, sales, anumber)
    ...
    ) art ON art.drky LIKE '%'||TRIM(itm.imsrp9)
    LEFT JOIN
    (
  ...
  def self.generate_pbj(branch, brand)
    Jde.find_by_sql("
      ...
      (
        ...
        AND IBMCU LIKE '%#{branch}' AND REGEXP_LIKE(IBSRP1, '#{brand}') GROUP BY IBITM, IBMCU
      ) IB
      FULL OUTER JOIN
      (
        SELECT SDITM, MAX(SDLITM) AS SDLITM, SDMCU, SUM(SDUORG) AS QTY, MIN(SDDRQJ) AS SO_REQDATE FROM PRODDTA.F4211 
        WHERE SDNXTR = '525' AND REGEXP_LIKE(SDDCTO,'SO|ZO') AND SDMCU LIKE '%#{branch}' AND 
        (REGEXP_LIKE(SDSRP1, '#{brand}') OR SDSRP1 LIKE 'K%') AND SDCOMM != 'K'
        GROUP BY SDITM, SDMCU
      ...
      (
        ...
        WHERE IMTMPL LIKE '%BJ MATRASS%' GROUP BY IMITM, IMLITM, IMDSC1, IMDSC2
  ...
  def self.outstand_order_sales(branch, brand, sales, anumber)
    Jde.find_by_sql("SELECT 
        ...
        so.sduorg AS quantity_outstanding,
        so.sdtrdj AS sdtrdj,
