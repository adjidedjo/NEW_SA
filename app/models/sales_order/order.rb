class SalesOrder::Order < ActiveRecord::Base
  ...
  def self.generate_pbj(branch, brand)
    Jde.find_by_sql("
      ...
      FULL OUTER JOIN
      (
        SELECT MAX(SDDOCO), SDITM, MAX(SDLITM) AS SDLITM, SDMCU, SUM(SDUORG) AS QTY, MIN(SDDRQJ) AS SO_REQDATE FROM PRODDTA.F4211 
        WHERE SDNXTR = '525' AND SDDCTO = 'SO' AND SDMCU LIKE '%#{branch}' AND 
        SDGLC LIKE '%L ' AND SDCOMM != 'K'
        GROUP BY SDITM, SDMCU ORDER BY SDDOCO
      ) BO ON IB.IBITM = BO.SDITM
      LEFT JOIN
      (
        SELECT LIITM, LIMCU, SUM(LIPQOH) AS ONHAND, SUM(LIHCOM) AS COMIT FROM PRODDTA.F41021 
        WHERE LIMCU LIKE '#{brand}%' AND LIPQOH > 0 GROUP BY LIITM, LIMCU
