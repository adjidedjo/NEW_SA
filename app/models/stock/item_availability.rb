class Stock::ItemAvailability < ActiveRecord::Base
  ...
  def self.stock_display_report(branch, brand)
    Jde.find_by_sql("
      SELECT MAX(SO.SDDELN) AS NOSJ, MAX(SO.SDDOCO) AS NOSO, MAX(PO.PHMCU) AS BRANCH, SA.LILOTN, NVL(NULLIF(MAX(SO.SDADDJ), 0),MAX(SO.SDDRQJ)) AS ACTUALSHIP, MAX(SO.SDSHAN) AS CUS, MAX(CU.ABALPH) CUSTOMER,
        MAX(SO.SDSRP1) AS BRAND, MAX(SO.SDLITM) AS SDLITM, MAX(SO.SDDSC1) AS DESC1, MAX(SO.SDDSC2) AS DESC2, MAX(SA.LIPQOH) AS JML, MAX(CM1.ABALPH) AS SALESMAN,
        SA.LILRCJ AS RECEIVE_DATE
      FROM 
      (
        SELECT LIMCU, MAX(LILOTN) AS LILOTN, SUM(LIPQOH) AS LIPQOH, MAX(LILRCJ) AS LILRCJ FROM PRODDTA.F41021
        WHERE LIMCU LIKE '%#{branch}%' AND LIPQOH >= 10000 AND LIPBIN = 'S' GROUP BY LIMCU, LILOTN
      ) SA
      LEFT JOIN
      (
        SELECT MAX(SDDELN) AS SDDELN, MAX(SDDOCO) AS SDDOCO, SDLOTN, SDSHAN, MAX(SDSRP1) AS SDSRP1, 
        ...
        GROUP BY SDLOTN, SDSHAN ORDER BY SDADDJ DESC
      ...
      WHERE SO.SDDOCO IS NOT NULL AND PO.PHMCU IS NOT NULL GROUP BY SA.LILOTN, SA.LILRCJ
    ")
  end
