class Stock::ItemAvailability < ActiveRecord::Base
  ...
  def self.stock_display_report(branch, brand)
    Jde.find_by_sql("
      SELECT MAX(SO.SDDELN) AS NOSJ, MAX(SO.SDDOCO) AS NOSO, MAX(SO.SDMCU) AS BRANCH, SA.LILOTN, 
        NVL(NULLIF(MAX(SO.SDADDJ), 0),MAX(SO.SDDRQJ)) AS ACTUALSHIP, MAX(SO.SDSHAN) AS CUS, CU.ABALPH AS CUSTOMER,
      ...
      GROUP BY SA.LILOTN, SA.LILRCJ, CU.ABALPH ORDER BY CU.ABALPH ASC
    ")
  end
