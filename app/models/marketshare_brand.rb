class MarketshareBrand < ActiveRecord::Base
  ...
  def self.customers(area, brand, user)
    find_by_sql("
      SELECT ms.customer_name, ms.city, ms.start_date, ms.end_date, ms.internal_brand, ms.name, SUM(ms.amount) AS amount FROM marketshare_brands ms WHERE ms.area_id = '#{area}' AND IF('#{user}' = 'admin', ms.internal_brand = 0, ms.internal_brand = '#{brand}')
