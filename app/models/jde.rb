class Jde < ActiveRecord::Base
  ...
  def self.calculate_pbjm(from, to)
    find_by_sql("
      SELECT PF.SDMCU, PF.PBJ, TRIM(PF.SDSHAN) AS SDSHAN, PF.DES, SUM(PF.TOTAL_PBJM) AS TOTAL, TRIM(PF.WC) AS WC, PF.STAT FROM(
        ...
        MAX(PBJ.IBSRP6) AS WC, (CASE WHEN PBJ.SDLTTR = 580 THEN 'SENT' ELSE 'OUTS' END) AS STAT FROM (
          SELECT ORD.*, IBC.IBSRP6 FROM(
            ...
            LEFT OUTER JOIN
            (
              SELECT IBITM, IBSRP6 FROM PRODDTA.F4102 WHERE REGEXP_LIKE(IBMCU, '11001|11002|12001|15151')
