class Jde < ActiveRecord::Base
  ...
  def self.calculate_pbjm(from, to, brand)
    brand = brand.at(0)
    find_by_sql("
      SELECT MAX(PF.SDMCU) AS SDMCU, PF.PBJ, TRIM(PF.SDSHAN) AS SDSHAN, PF.DES, SUM(PF.TOTAL_PBJM) AS TOTAL, PF.STAT FROM(
      ...
      GROUP BY PF.SDSHAN, PF.PBJ, PF.DES, PF.STAT ORDER BY PF.SDSHAN
    ")
  end
