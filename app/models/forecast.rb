class Forecast < ActiveRecord::Base
  ...
  def self.calculation_forecasts_by_branch_and_sales(start_date, end_date, area)
    self.find_by_sql("
      ...
      GROUP BY f.address_number, f.brand
      ) AS oa GROUP BY oa.address_number, oa.jenisbrgdisc
    ")
  end
  ...
  def self.calculation_forecasts_salesman(start_date, end_date, area, brand)
    self.find_by_sql("
      SELECT f.item_number, f.description, f.segment1, f.segment2_name, f.brand, f.month, f.year,
